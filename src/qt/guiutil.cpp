...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Cognitio address (e.g. %1)").arg("AWSbBnzmNkjDVaYHX7vkL1MqD96pRYWtZo"));
#endif
...
{
    // return if URI is not valid or is no Cognitio: URI
...
{
    // Convert cognitio:// to cognitio:
    //
    //    Cannot handle this later, because cognitio:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open cognitio.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Cognitio.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Cognitio.lnk
...
{
    return GetAutostartDir() / "cognitio.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a cognitio.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Cognitio\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the cognitio app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add cognitio app to startup item list
